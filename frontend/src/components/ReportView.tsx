...
export default function ReportView({ report }: { report: AnalysisReport }) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-5">
      ...
      <div className={`${v.bg} ${v.border} border-2 rounded-2xl p-6 shadow-sm`}>
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <div className="text-xs uppercase tracking-wider text-neutral-500 mb-1">Вердикт анализа</div>
            <div className={`text-2xl font-bold ${v.text}`}>{v.label}</div>
            {report.verdict_condition && (
              <div className="text-sm mt-2 text-neutral-700">Условие: {report.verdict_condition}</div>
            )}
          </div>
        </div>
        <div className="flex flex-wrap gap-3 mt-5">
          <KPI label="Сегментов" value={String(report.segments.length)} />
          <KPI label="Приоритет (A)" value={String(aCount)} sub="≥70 баллов" />
          <KPI label="Второй эшелон (B)" value={String(bCount)} sub="50-69" />
      ...
      {/* ── Segments ── */}
      {segmentsSorted.length > 0 && (
        <Section title={`Сегменты аудитории (${segmentsSorted.length})`} icon="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z">
      ...
      {report.top_risks.length > 0 && (
        <Section title={`Рискованные допущения (${report.top_risks.length})`} icon="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126ZM12 15.75h.007v.008H12v-.008Z">
          <p className="text-xs text-neutral-500 mb-3">
            Что мы предполагаем как правду, но не проверили. Чем выше балл (P×I) — тем опаснее ошибиться. Для каждого предложен эксперимент на 30 дней.
          </p>
          <div className="space-y-2">
              ...
              <div key={i} className="flex items-start gap-3 p-3 rounded-xl bg-neutral-50 border border-neutral-100">
                ...
                <div className="flex-1 text-sm">
                  ...
                  <div className="text-neutral-400 text-xs mb-1.5">
                    Вероятность провала: {r.probability}/5 · Последствия: {r.impact}/5
                  </div>
                  {r.experiment && (
        ...
        <Section title={`Конкуренты (${report.competitors.length})`} icon="M3.75 21h16.5M4.5 3h15M5.25 3v18m13.5-18v18M9 6.75h1.5m-1.5 3h1.5m-1.5 3h1.5m3-6H15m-1.5 3H15m-1.5 3H15M9 21v-3.375c0-.621.504-1.125 1.125-1.125h3.75c.621 0 1.125.504 1.125 1.125V21">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {report.competitors.map((c, i) => (
              <div key={i} className="border border-neutral-200 rounded-xl p-3.5 hover:shadow-sm transition-shadow">
                ...
                {c.url && (
                  <a href={c.url} target="_blank" rel="noreferrer"
                    className="text-xs hover:underline cursor-pointer" style={{ color: C.secondary }}>{c.url}</a>
                )}
                {c.unmet_job && (
